...
export default function HockeyLeft() {
  ...
  const handleSearch = async () => {
    const res = axios.get(
      `https://v1.hockey.api-sports.io/teams?search=${debounceSearch}`,
      {
        headers: {
  ...
  const handleDefaultTeams = async () => {
    const res = axios.get(
      `https://v1.hockey.api-sports.io/teams?league=${majorLeagueId.HOC}&season=${season}`,
      {
        headers: {
          "x-apisports-key ": xxxKey,
        },
  ...
  console.log("log out hoc search data ", searchTeamData);
  console.log("log out hoc the  default data ", defaultTeamData);

  console.log("log out hoc the data", toShowTeam);
  return (
    <>
